...
namespace Liduv.Setting
{
  ...
  public class Configuration : DependencyObject, INotifyPropertyChanged
  {
    ...
    ///   local application data e.g. C:\Users\%Username%\AppData\Roaming\Liduv
    /// </returns>
    public static string GetLocalApplicationDataPath()
    {
      ...
      localApplicationData += Path.DirectorySeparatorChar + "Liduv";
    ...
    /// <summary>
    /// Returns the path to the Liduv directory in the
    ///   local application data folder of the user, which is writable
    ///   even if the user has not admin rights.
    /// </summary>
    /// <returns>
    /// A <see cref="string"/> with the path to users
    /// MyDocuments data e.g. C:\Users\%Username%\MyDocuments\Liduv
    /// </returns>
    public static string GetMyDocumentsPath()
    {
      var myDocuments = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
      myDocuments += Path.DirectorySeparatorChar + "Liduv";
